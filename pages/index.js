...
export default function Home() {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth <= 640) {
        githubImgRef.current.style.width = "20px";
        googleImgRef.current.style.width = "20px";
      } else {
        githubImgRef.current.style.width = "14px";
        googleImgRef.current.style.width = "14px";
      }
    };
  ...
  return (
    <div
      className={`${styles.home_container} w-screen h-screen flex md:flex-row flex-col`}
    >
      <div
      ...
      >
        <div className={`w-[90%] md:max-w-[385px] flex flex-col gap-y-[10px]`}>
          ...
          <div className={`flex gap-x-[20px] mb-[8px]`}>
            ...
            >
              <img width="14px" src="/static/google.png" ref={googleImgRef} />
              <div className="hidden sm:block">Sign in with Google</div>
              <div className="sm:hidden text-lg">Google</div>
            </Link>
            <Link
              href="/"
              onClick={() => {
              ...
              className={`bg-white px-[15px] py-[5px] text-mid rounded-lg text-[#858585] flex-auto flex items-center justify-center gap-x-[12px] tracking-[0.2px]`}
            >
              <img width="14px" src="/static/github.png" ref={githubImgRef} />
              <div className="hidden sm:block">Sign in with GitHub</div>
              <div className="sm:hidden text-lg">GitHub</div>
            </Link>
          </div>
          <form
          ...
          >
            ...
            <div className={`md:text-base text-formblue`}>Forgot Password?</div>
            <button
              type="submit"
